...
//
// 1. the server `enschedule` probably takes a `ConnectionId` indicating to whom the packet will be
// sent;
fn server_main() {
    ...
    loop {
        if d_counter % 8 == 0 {
            // TODO(alex) 2021-05-18: Should we allow packets to be enscheduled before the connection
            // is properly estabilished?
            let packet_id = server.enschedule((&d_counter.to_be_bytes()).to_vec());
